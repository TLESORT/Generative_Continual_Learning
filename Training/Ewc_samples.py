...
class Ewc_samples(Trainer):
    def __init__(self, model, args):
        super(Ewc_samples, self).__init__(model, args)
        self.params = None
        self._means = None
    ...
    def _diag_fisher(self, model):
        ...
        if self.context == 'Generation':
            ...
            if model.model_name in ['CVAE', 'VAE']:
                ...
                for iter, (x_, t_) in enumerate(self.old_task):

                    self.model.E_optimizer.zero_grad()
                    self.model.G_optimizer.zero_grad()

                    x_ = variable(x_).view(1, 784)
    ...
    def preparation_4_task(self, model, ind_task):
        ...
        if ind_task > 0 and self.context == 'Generation':
            ...
            # Here model is generator + discriminator/encoder.
            ...
            old_tasks = []
            for sub_task in range(self.ind_task + 1):
                data = list(self.train_loader[sub_task].get_sample(self.samples_per_task))
                data = [(data[0][i], torch.LongTensor([data[1][i]])) for i in range(len(data[0]))]
                old_tasks = old_tasks + data
            #old_tasks = random.sample(old_tasks, k=self.samples_per_task)
            self.old_task = old_tasks
            ...
            # generate dataset for later evaluation
...
'''
        ...
        if ind_task > 0 and self.context == 'Generation':
            ...
            self.model.generate_dataset(ind_task - 1, nb_sample_train, nb_sample_test, one_task=False)
